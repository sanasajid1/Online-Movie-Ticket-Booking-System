 ...
 * @author abc
 */
@WebServlet(name = "cinemasServlet", urlPatterns = {"/cinemasServlet"})
public class cinemasServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet cinemasServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet cinemasServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        {
            obj.getTicket().setCinema(obj.getCinemaFromList("Cinepax"));
            System.out.println("PRINTING FROM CONSOLE"+obj.getTicket().getCinema().getName());
            PrintWriter writer = response.getWriter();
                RequestDispatcher requestDispatcher = request.getRequestDispatcher("genre.jsp");
                requestDispatcher.forward(request, response);
        ...
        {
            obj.getTicket().setCinema(obj.getCinemaFromList("Cinestar"));
            System.out.println("PRINTING FROM CONSOLE"+obj.getTicket().getCinema().getName());
            PrintWriter writer = response.getWriter();
                RequestDispatcher requestDispatcher = request.getRequestDispatcher("genre.jsp");
                requestDispatcher.forward(request, response);
        ...
        {
            obj.getTicket().setCinema(obj.getCinemaFromList("Super Cinema"));
            System.out.println("PRINTING FROM CONSOLE"+obj.getTicket().getCinema().getName());
            PrintWriter writer = response.getWriter();
                RequestDispatcher requestDispatcher = request.getRequestDispatcher("genre.jsp");
                requestDispatcher.forward(request, response);
